<template>
    <div
        class="manage manage-game-cube"
        :style="{
            '--piece-font-size': size,
            '--length-x': X + 'em',
            '--length-y': Y + 'em',
            '--length-z': Z + 'em',
            '--bg-translate': `translate3d(-${X/2}em, -${Y/2}em, -${Z/2}em)`
        }"
    >
        <div class="game-control">

        </div>
        <div class="game-panel">
            <div class="cube-box">
                <cube-piece
                    v-for="(item, index) in positions"
                    :key="index"
                    :opacity="opacitys[index]"
                    :position="positions[index]"
                    :size="size"
                />
                <div class="inner-bg inner-bg-x"></div>
                <div class="inner-bg inner-bg-y"></div>
                <div class="inner-bg inner-bg-z"></div>
            </div>
        </div>
    </div>
</template>

<script>
    import cubePiece from './piece.vue'
    export default {
        name: 'manage-game-cube',
        components: {
            cubePiece
        },
        data () {
            return {
                // 阶数
                X: 3,
                Y: 3,
                Z: 3,
                // 每块大小
                size: '15px',
                // 原始位置
                origin: '',
                // 小方位置集合
                positions: [],
                // 小方6面显示状态集合
                opacitys: []
            }
        },
        created () {
            this.paint()
        },
        methods: {
            paint () {
                // 前 后 上 下 左 右
                const temp = [0, 0, 0, 0, 0, 0]
                // 初始化每一个小方块的颜色和位置
                for (let i = 1; i < this.Z + 1; i++) {
                    temp[0] = i === 1 ? 1 : 0
                    temp[1] = i === this.Z ? 1 : 0

                    for (let j = 1; j < this.Y + 1; j++) {
                        temp[2] = j === 1 ? 1 : 0
                        temp[3] = j === this.Y ? 1 : 0

                        for (let k = 1; k < this.X + 1; k++) {
                            temp[4] = k === 1 ? 1 : 0
                            temp[5] = k === this.X ? 1 : 0
                            // 根据中心点偏移
                            this.positions.push([k - 1 - this.X / 2, j - 1 - this.Y / 2, this.Z / 2 - i - 1])
                            // 颜色配置
                            this.opacitys.push(temp.join(''))
                        }
                    }
                }
                // 存储初始位置状态
                this.origin = this.positions.flat().join()
            }
        }
    }
</script>

<style lang="scss" src="./assets/css/index.scss"></style>
